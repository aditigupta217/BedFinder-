...
function HospitalPortal() {
  ...
  const hospital = useMemo(
    () => hospitals.find((h) => h.id === authedId) ?? null,
    [hospitals, authedId],
  );
...
function LoginCard({ hospitals, onAuth }: { hospitals: Hospital[]; onAuth: (id: string) => void }) {
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] flex items-center justify-center px-4 py-10 pb-20">
      <div className="w-full max-w-md bg-white border border-[#E8ECF2] rounded-2xl p-6 sm:p-8 shadow-sm fade-up">
        <div className="inline-flex items-center gap-2 rounded-full bg-[#E8341C]/10 text-[#E8341C] px-3 py-1 text-xs font-semibold uppercase tracking-wider">
          {t("Hospital Staff Only")}
        </div>
        <h1 className="font-display font-bold text-2xl sm:text-3xl text-[#0A1628] mt-3">
          {t("Hospital Portal")}
        </h1>
        ...
        <label className="block text-xs font-medium text-[#0A1628] mt-5 mb-1.5">{t("PIN")}</label>
        ...
        <button
          onClick={submit}
          className="mt-6 w-full bg-[#0A1628] hover:bg-[#15233f] text-white font-medium py-3 rounded-lg transition-colors cursor-pointer"
        >
          {t("Sign in →")}
        </button>
...
function StaffDashboard({ hospital, onSignOut }: { hospital: Hospital; onSignOut: () => void }) {
  ...
  const save = () => {
    ...
    toast.success(t("Saved! Citizens can see updated counts."));

    // Check waiting list and notify using EmailJS (for city matching hospital.city and icu > 0)
    if (icu > 0) {
      const count = hospitalsStore.notifyWaitingUsers(
        hospital.city,
        hospital.name,
        icu,
        gen,
      ...
      if (count > 0) {
        toast.success(
          t("✅ Notification sent to [X] waiting users!").replace("[X]", String(count)),
        );
      }
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="font-display font-bold text-2xl sm:text-3xl text-[#0A1628]">
            {hospital.name}
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            ID: {hospital.id} · {hospital.city}
          </p>
        </div>
        <button
          onClick={onSignOut}
          className="text-sm text-[#0A1628] hover:underline cursor-pointer"
        >
          {t("Sign out")}
        </button>
      </div>
      ...
      {stale && (
        <div className="mt-4 bg-[#D97706]/10 border border-[#D97706]/30 text-[#92400e] rounded-lg px-4 py-3 text-sm">
          {t("Last update was more than 2 hours ago. Please refresh counts.")}
        </div>
      )}

      <div className="mt-6 bg-white border border-[#E8ECF2] rounded-xl p-5 sm:p-6 fade-up">
        <h3 className="font-display font-semibold text-[#0A1628]">
          {t("Update bed availability")}
        </h3>
        <p className="text-xs text-muted-foreground mt-1">
          {t("updated_x_mins_ago", { time: minutesAgo(hospital.last_updated, t) })}
        </p>
        ...
        <div className="grid sm:grid-cols-3 gap-4 mt-5">
          <Counter
            label="ICU"
            value={icu}
            setValue={(v) => setIcu(clamp(v, hospital.icu_total))}
            total={hospital.icu_total}
          />
          <Counter
            label="General"
            value={gen}
            setValue={(v) => setGen(clamp(v, hospital.general_total))}
            total={hospital.general_total}
          />
          <Counter
            label="Emergency"
            value={emr}
            setValue={(v) => setEmr(clamp(v, hospital.emergency_total))}
            total={hospital.emergency_total}
          />
        </div>

        <button
          onClick={save}
          className="mt-6 w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-[#16A34A] hover:bg-[#15803d] text-white font-medium px-5 py-2.5 rounded-lg transition-colors cursor-pointer"
        >
          {t("✓ Save — go live now")}
        </button>
      </div>
      ...
      <div className="mt-6 bg-white border border-[#E8ECF2] rounded-xl p-5 sm:p-6">
        <h3 className="font-display font-semibold text-[#0A1628]">{t("Today's update history")}</h3>
        {log.length === 0 ? (
          <p className="text-sm text-muted-foreground mt-3">{t("No updates yet this session.")}</p>
        ) : (
            ...
            {log.map((e, i) => (
              <li key={i} className="py-2.5 flex items-center justify-between text-sm">
                <span className="text-[#0A1628] font-medium">
                  {t("ICU")} {e.icu} · {t("General")} {e.general} · {t("Emergency")} {e.emergency}
                </span>
    ...
    <div className="bg-[#F4F6F9] border border-[#E8ECF2] rounded-xl p-4">
      ...
      <div className="mt-2 flex items-center justify-between gap-3">
        <button
          ...
          className="h-10 w-10 rounded-lg bg-white border border-[#E8ECF2] hover:bg-[#F4F6F9] text-lg font-semibold text-[#0A1628] cursor-pointer"
        >
          −
        </button>
        <input
          ...
          className="w-full text-center text-2xl font-display font-bold text-[#0A1628] bg-transparent outline-none border-none focus:ring-0"
        />
        <button
          onClick={() => setValue(value + 1)}
          className="h-10 w-10 rounded-lg bg-white border border-[#E8ECF2] hover:bg-[#F4F6F9] text-lg font-semibold text-[#0A1628] cursor-pointer"
